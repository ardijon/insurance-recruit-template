...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "خطا در خواندن دیوار موفقیت" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  if (body.agent_name.length > 100) {
    return NextResponse.json({ error: "agent_name must be 100 characters or less" }, { status: 422 });
  }
  if (body.quote.length > 500) {
    return NextResponse.json({ error: "quote must be 500 characters or less" }, { status: 422 });
  }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await ensureSchema();
    const result = await executeUpdate("DELETE FROM success_wall_entries WHERE id = ?", [Number(id)]);
